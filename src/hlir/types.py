...
HLIR_VALUE_OP_LOGIC_OR = 'logical-or'
HLIR_VALUE_OP_LOGIC_AND = 'logical-and'
HLIR_VALUE_OP_LOGIC_NOT = 'logical-not'

HLIR_VALUE_OP_BITWISE_AND = 'bitwise-and'
HLIR_VALUE_OP_BITWISE_XOR = 'bitwise-xor'
HLIR_VALUE_OP_BITWISE_OR = 'bitwise-or'
HLIR_VALUE_OP_BITWISE_NOT = 'bitwise-not'
...
HLIR_VALUE_OP_SIZEOF_TYPE = 'sizeof-type'
HLIR_VALUE_OP_SIZEOF_VALUE = 'sizeof-value'
HLIR_VALUE_OP_ALIGNOF_TYPE = 'alignof-type'
HLIR_VALUE_OP_ALIGNOF_VALUE = 'alignof-value'
HLIR_VALUE_OP_LENGTHOF_TYPE = 'lengthof-type'
HLIR_VALUE_OP_LENGTHOF_VALUE = 'lengthof-value'
HLIR_VALUE_OP_OFFSETOF = 'offsetof'
HLIR_VALUE_OP_ACCESS_MODULE ='access-module'
HLIR_VALUE_OP_DEFINED_TYPE = 'defined-type'
HLIR_VALUE_OP_DEFINED_VALUE = 'defined-value'
...
ARITHMETICAL_OPS = (HLIR_VALUE_OP_ADD, HLIR_VALUE_OP_SUB, HLIR_VALUE_OP_MUL, HLIR_VALUE_OP_DIV, HLIR_VALUE_OP_NEG, HLIR_VALUE_OP_POS)
REM_OP = (HLIR_VALUE_OP_REM,)
...
INT_OPS = CONS_OP + EQ_OPS + RELATIONAL_OPS + ARITHMETICAL_OPS + REM_OP
NAT_OPS = CONS_OP + EQ_OPS + RELATIONAL_OPS + ARITHMETICAL_OPS + REM_OP
...
class Type(Entity):
	...
	def is_empty_array(self):
		if self.is_array():
			return self.volume.is_immediate() and self.volume.asset == 0
		return False
	...
	def eq_array(a, b, opt):
		...
		if a.volume.is_immediate() and b.volume.is_immediate():
	...
	def eq(a, b, opt=[], debug=False):
		...
		elif a.is_variant(): return Type.eq_variant(a, b, opt)
...
class TypeRational(TypeSimple):
	def __init__(self, ti=None):
		...
		super().__init__(width=0, kind=HLIR_TYPE_KIND_INTEGER, id=rational_id, ops=RATIONAL_OPS, ti=ti)
		self.generic = True
...
class Value(Entity):
	...
	def is_immutable(self):
		...
		return (not self.isLvalue()) or self.immutable or self.hasAttribute('immutable')

	def is_linktime(self):
	...
	def is_zero(self):
		...
		if self.type.is_record():
			for initializer in self.asset:
				if not initializer.value.is_zero():
					return False
			return True
...
class ValueArray(Value):
	...
	def eq(l, r, ti):
		...
		if not (l.is_immediate() and r.is_immediate()):
			return False

		lvolume = l.type.volume
		rvolume = r.type.volume
		if not (lvolume.is_immediate() and rvolume.is_immediate()):
			return False
