...
static struct main_context main_init(uint8_t key[32], uint32_t _nonce[3]) {
	uint32_t nonce[3];
	__builtin_memcpy(nonce, _nonce, sizeof(uint32_t [3]));
	return (struct main_context){
		.key = key,
...
static void main_cipher(struct main_context *ctx, uint8_t data[], uint32_t len) {
	uint32_t i = 0U;
	uint8_t (*bptr)[] = NULL;
	while (i < len) {
...
static uint8_t main_testKey[32] = {
	0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7,
	0x8, 0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF,
	0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
	0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F
};
static uint8_t main_testNonce[12] = {
	0x0, 0x0, 0x0, 0x9,
	0x0, 0x0, 0x0, 0x4A,
	0x0, 0x0, 0x0, 0x0
};
static uint32_t main_testNonce2[3] = {
	0x9U,
	0x4AU,
	0x0U
};
#define MAIN_TEST_RESULT { \
...
static bool main_test0(void) {
	uint8_t key[32];
	__builtin_memcpy(&key, &main_testKey, sizeof(uint8_t [32]));
	uint32_t counter = 0x1U;
	uint8_t nonce[12];
	__builtin_memcpy(&nonce, &main_testNonce, sizeof(uint8_t [12]));
	chacha20_State state;
