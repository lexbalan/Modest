...
| Semantic analysis / new statement | `src/semantic.py` |
...
3. Translate in `src/semantic.py` → return a `StmtXxx` HLIR node (define in `src/hlir/types.py`)
...
4. Translate in `src/semantic.py` → return `ValueBin(type, op, left, right, ti)`
...
3. Register in `src/semantic.py` initial symbol table setup
...
- Backends never call `error()` — all errors come from `semantic.py`
